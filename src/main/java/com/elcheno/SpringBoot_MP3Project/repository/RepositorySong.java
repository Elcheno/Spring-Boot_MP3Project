...
public interface RepositorySong extends JpaRepository<Song, Long> {
    ...
    @Query(value = "SELECT s.id, s.artist, s.description, s.title, s.url, s.img FROM song s JOIN listas l JOIN listas_songs ls ON s.id=ls.songs_id AND l.id=ls.lista_song_id WHERE l.id=?1", nativeQuery = true)
